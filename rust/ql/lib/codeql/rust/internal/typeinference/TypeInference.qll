...
private module AssocFunctionResolution {
  ...
  private predicate traitSelfTypeParameterOccurrence(
    TraitItemNode trait, NonMethodFunction f, FunctionPosition posAdj
  ) {
    exists(FunctionPosition pos |
  ...
  private predicate traitImplSelfTypeParameterOccurrence(
    ImplItemNode impl, NonMethodFunction f, FunctionPosition posAdj
  ) {
    exists(NonMethodFunction traitFunction |
      f = impl.getAnAssocItem() and
      f.implements(traitFunction) and
      traitSelfTypeParameterOccurrence(_, traitFunction, posAdj)
    )
  }
  ...
  private predicate assocFunctionTraitInfo(string name, int arity, Trait trait) {
    exists(ImplItemNode i |
      assocFunctionInfo(_, name, arity, _, i, _, _, _, _, _, _) and
      trait = i.resolveTraitTy()
    )
    or
    assocFunctionInfo(_, name, arity, _, trait, _, _, _, _, _, _)
  }
  ...
  private predicate nonBlanketLikeCandidate(
    ...
    |
      assocFunctionInfoNonBlanketLikeCand(f, name, arity, selfPosAdj, i, selfType, strippedTypePath,
        strippedType, typeQualifier, traitQualifier, hasReceiver)
      or
      assocFunctionInfoNonBlanketLikeTypeParamCand(f, name, arity, selfPosAdj, i, selfType,
  ...
  abstract class AssocFunctionCall extends Expr {
    ...
    /**
     * Holds if `selfPosAdj` is a potentially relevant position for resolving this call.
     */
    pragma[nomagic]
     ...
     * by `derefChain` and `borrow` is incompatible with the type at `selfPosAdj`.
     *
     ...
     * at `selfPosAdj`, or because the blanket constraint is not satisfied.
     */
    pragma[nomagic]
     ...
     * have a matching call target at `selfPosAdj`.
     */
    pragma[nomagic]
     ...
     * a matching non-blanket call target at `selfPosAdj`.
     */
    pragma[nomagic]
     ...
     * by a shared borrow, does not have a matching call target at `selfPosAdj`.
     */
    pragma[nomagic]
    ...
    /**
     * Holds if the candidate receiver type represented by `derefChain`, followed
     * by a `mut` borrow, does not have a matching call target at `selfPosAdj`.
     */
    pragma[nomagic]
    ...
    /**
     * Holds if the candidate receiver type represented by `derefChain`, followed
     * by a `mut` borrow, does not have a matching non-blanket call target at `selfPosAdj`.
     */
    pragma[nomagic]
    ...
    /**
     * Gets the type of this call at `selfPosAdj` and `path`.
     *
   ...
   * a function at a position relevant for dispatch (such as a `self` parameter).
   */
   ...
   * a function at a position relevant for dispatch (such as a `self` parameter) in a
   * blanket (like) implementation.
   */
  ...
  /**
   * A configuration for anti-matching the type of an argument against the type of
   * a function at a position relevant for dispatch (such as a `self` parameter) in an
   * inherent function.
   */
