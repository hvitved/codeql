...
signature module InputSig<DF::InputSig Lang> {
  ...
  /**
   * Gets the content corresponding to the unknown token `token`.
   *
   * The token is unknown because it could not be reverse-encoded using the
   * encoding predicates above.
   */
  bindingset[token]
  ...
  /**
   * Gets the return kind corresponding to the unknown token `token`.
   *
   * The token is unknown because it could not be reverse-encoded using the
   * encoding predicates above.
   */
  ...
  /**
   * Gets the without-content corresponding to the unknown token `token`.
   *
   * The token is unknown because it could not be reverse-encoded using the
   * encoding predicates above.
   */
  ...
  /**
   * Gets the with-content corresponding to the unknown token `token`.
   *
   * The token is unknown because it could not be reverse-encoded using the
   * encoding predicates above.
   */
...
module Make<DF::InputSig DataFlowLang, InputSig<DataFlowLang> Input> {
  ...
  module Private {
     ...
     */
    module External {
      private SummaryComponent decodeToken(AccessPathToken token) {
        exists(ContentSet c |
          exists(string name | name = encodeContent(c, token.getAnArgument(name)))
          or
          token = encodeContent(c, "")
        |
        ...
        or
        exists(ReturnKind rk |
          exists(string name | name = encodeReturn(rk, token.getAnArgument(name))) or
          token = encodeReturn(rk, "")
        |
        ...
        or
        exists(ContentSet c |
          exists(string name | name = encodeWithoutContent(c, token.getAnArgument(name)))
          or
          token = encodeWithoutContent(c, "")
        |
          result = SummaryComponent::withoutContent(c)
        )
        or
        exists(ContentSet c |
          exists(string name | name = encodeWithContent(c, token.getAnArgument(name))) or
          token = encodeWithContent(c, "")
        |
      ...
      private SummaryComponent interpretComponent(AccessPathToken token) {
        ...
        or
        not exists(decodeToken(token)) and
        (
          result = SummaryComponent::content(decodeUnknownContent(token))
          or
          result = SummaryComponent::return(decodeUnknownReturn(token))
          or
          result = SummaryComponent::withoutContent(decodeUnknownWithoutContent(token))
          or
          result = SummaryComponent::withContent(decodeUnknownWithContent(token))
        )
      }
      ...
      signature module SourceSinkInterpretationInputSig {
         ...
         */
        predicate sourceElement(Element n, string output, string kind, string provenance);

        /**
         * Holds if an external sink specification exists for `n` with input specification
         * `input`, kind `kind` and provenance `provenance`.
         */
        predicate sinkElement(Element n, string input, string kind, string provenance);
      ...
      /**
       * Legacy interface for interpreting source/sink specifications in Java and C#.
       *
        ...
        SourceSinkInterpretationInputSig SourceSinkInterpretationInput>
      {
        ...
        private predicate outputNeedsReferenceExt(AccessPathToken c) {
          ...
          |
            exists(ArgumentPosition apos, ParameterPosition ppos |
              node.asNode()
                  ...
                  .getPreUpdateNode()
                  .(ArgNode)
                  .argumentOf(mid.asCall(), apos) and
