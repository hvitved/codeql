 ...
 * 2. assign `Default::default()` the special `UnknownType` (this library has built-in
 *    logic for identifying calls where (parts of) the return type needs to be inferred
 ...
 * contextual inference will then propagate the `i64` type backwards using two
 * reversed steps.
 */
overlay[local?]
module;
...
module Make1<LocationSig Location, InputSig1<Location> Input1> {
  ...
  module Make2<HasTypeTreeSig TypeMention, InputSig2<TypeMention> Input2> {
    ...
    signature module InputSig3 {
      ...
      /**
       * This predicate must be implemented as an alias for the `inferType` predicate
      ...
      class Invocation extends Expr {
         ...
         * arguments that are part of a type qualifier (those should be handled via
         * `getTypeQualifier`).
         *
         * Example:
         *
    ...
    module Make3<InputSig3 Input3> {
      ...
      private module Certain {
        ...
        predicate stepCertain(AstNode n1, TypePath prefix1, AstNode n2, TypePath prefix2) {
          // language-specific inference steps are assumed to preserve certainty when
          // they have a unique premise
          stepLanguageSpecific(n1, prefix1, n2, prefix2) and
          strictcount(AstNode n1_, TypePath prefix1_, TypePath prefix2_ |
            stepLanguageSpecific(n1_, prefix1_, n2, prefix2_)
          ) = 1
          or
          prefix1.isEmpty() and
          prefix2.isEmpty() and
        ...
        /**
         * Gets the inferred certain type of `n` at `prefix`, where `prefix` is a proper
         * prefix of a unique bottom-up inference step for `n`.
         *
         * For example, if we have a rule for Rust array repeat expressions like
         *
         * ```text
         *       e : T
         * --------------------
         ...
         * ```
         *
         * then this predicate allows us to infer that the root type of `[e; n]` is
         * `[...; n]`, regardless of whether we are able to infer the type of `e`.
         */
        pragma[nomagic]
        private Type inferTypeFromStepPrefixCertain(AstNode n, TypePath prefix) {
          exists(TypePath path, TypeParameter tp |
            path.isSnoc(prefix, tp) and
            result.getATypeParameter() = tp
          |
            path =
              unique(TypePath path0 |
                stepLanguageSpecific(_, TypePath::nil(), n, path0)
                or
                stepCertain(_, TypePath::nil(), n, path0)
                or
                closureStep(_, TypePath::nil(), n, path0)
              )
            or
            exists(inferTypeFromStepPrefixCertain(n, path))
          )
        }
        ...
        Type inferTypeCertain(AstNode n, TypePath path) {
          (
            ...
            or
            result = inferTypeFromStepPrefixCertain(n, path)
            or
      ...
      /**
       * Holds if the textual representation `repr` should be used for `n` in
       * `TypeTest`.
       */
