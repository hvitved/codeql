...
class OverloadByType {
  func handle(_ x: Int) -> String { // name=OverloadByType.handle(_:Int)
    return "int"
  }

  func handle(_ x: Double) -> String { // name=OverloadByType.handle(_:Double)
    return "double"
  }

  func handle(_ x: String) -> String { // name=OverloadByType.handle(_:String)
    return "string"
  }

  func handle(_ x: Bool) -> String { // name=OverloadByType.handle(_:Bool)
    return "bool"
  }
}
...
class OverloadByLabel {
  func configure(width: Int) -> String { // name=OverloadByLabel.configure(width:)
    return "width"
  }

  func configure(height: Int) -> String { // name=OverloadByLabel.configure(height:)
    return "height"
  }

  func configure(width: Int, height: Int) -> String { // name=OverloadByLabel.configure(width:height:)
    return "both"
  }

  func configure(size: Int) -> String { // name=OverloadByLabel.configure(size:)
    return "size"
  }
}
...
class OverloadByArity {
  func compute() -> Int { // name=OverloadByArity.compute()
    return 0
  }

  func compute(_ x: Int) -> Int { // name=OverloadByArity.compute(_:Int)
    return x
  }

  func compute(_ x: Int, _ y: Int) -> Int { // name=OverloadByArity.compute(_:Int,_:Int)
    return x + y
  }

  func compute(_ x: Int, _ y: Int, _ z: Int) -> Int { // name=OverloadByArity.compute(_:Int,_:Int,_:Int)
    return x + y + z
  }
}
...
class OverloadByReturn {
  func create() -> Int { // name=OverloadByReturn.create()->Int
    return 0
  }

  func create() -> String { // name=OverloadByReturn.create()->String
    return ""
  }

  func create() -> Double { // name=OverloadByReturn.create()->Double
    return 0.0
  }
}
...
class OverloadGenericVsConcrete {
  func process(_ x: Int) -> String { // name=OverloadGenericVsConcrete.process(_:Int)
    return "concrete"
  }

  func process<T>(_ x: T) -> String { // name=OverloadGenericVsConcrete.process<T>(_:)
    return "generic"
  }
}
...
func freeOverload(_ x: Int) -> String { // name=freeOverload(_:Int)
  return "int"
}

func freeOverload(_ x: String) -> String { // name=freeOverload(_:String)
  return "string"
}

func freeOverload(_ x: Double) -> String { // name=freeOverload(_:Double)
  return "double"
}
...
class MultiInit {
  ...
  init() { // name=MultiInit.init()
    value = "default"
  }

  init(int: Int) { // name=MultiInit.init(int:)
    value = "int"
  }

  init(str: String) { // name=MultiInit.init(str:)
    value = str // $ type=str:String
  }

  init(x: Int, y: Int) { // name=MultiInit.init(x:y:)
    value = "pair"
  }

  func getValue() -> String { // name=MultiInit.getValue
...
class StaticVsInstance {
  func action() -> String { // name=StaticVsInstance.action()->instance
    return "instance"
  }

  static func action() -> String { // name=StaticVsInstance.action()->static  
    return "static"
  }
...
extension Describable {
  func describe() -> String { // name=Describable.describe(default)
    return "default"
  }
...
class LabelVariants {
  func send(to target: String) -> String { // name=LabelVariants.send(to:)
    return target // $ type=target:String
  }

  func send(from source: String) -> String { // name=LabelVariants.send(from:)
    return source // $ type=source:String
  }

  func send(to target: String, from source: String) -> String { // name=LabelVariants.send(to:from:)
...
protocol Numeric2 : Equatable {
  static func zero() -> Self // name=Numeric2.zero
}

extension Int : Numeric2 {
  static func zero() -> Int { return 0 } // name=Int.zero
}

extension Double : Numeric2 {
  static func zero() -> Double { return 0.0 } // name=Double.zero
}

func constrainedId<T: Numeric2>(_ x: T) -> T { // name=constrainedId(_:Numeric2)
  return x
}

func constrainedId<T: Equatable>(_ x: T) -> T { // name=constrainedId(_:Equatable)
  return x
}
...
class Widget {
  ...
  init(name: String, size: Int) { // name=Widget.init(name:size:)
    self.name = name // $ type=name:String
    self.size = size // $ type=size:Int
  }

    
  convenience init(name: String) { // name=Widget.init(name:)
    self.init(name: name, size: 1) // $ target=Widget.init(name:size:)
  }

  convenience init(size: Int) { // name=Widget.init(size:)
...
class Processor {
  ...
  func apply(_ f: (Int) -> Int) -> Int { // name=Processor.apply(_:(Int)->Int)
    return f(0)
  }

  func apply(_ f: (String) -> String) -> String { // name=Processor.apply(_:(String)->String)
    return f("")
  }

  func apply(_ f: (Int, Int) -> Int) -> Int { // name=Processor.apply(_:(Int,Int)->Int)
    return f(0, 0)
  }
}
...
class MultiSubscript {
  ...
  func get(_ index: Int) -> Int { // name=MultiSubscript.get(_:)
    return data[index]
  }

  func get(_ key: String) -> Int { // name=MultiSubscript.get(_:String)
    return dict[key] ?? 0
  }
}
