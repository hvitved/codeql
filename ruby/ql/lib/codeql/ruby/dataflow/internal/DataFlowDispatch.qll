...
private module Cached {
  ...
  private predicate selfInSingletonMethodFlowsToMethodCallReceiver(
    RelevantCall call, Module m, string method, boolean exact
  ) {
    exists(SsaSelfDefinitionNode self, MethodBase caller |
      ...
      // For now, do not consider overrides inside blocks, for performance reasons
      if self.getCfgScope() instanceof Block then exact = true else exact = false
    )
  }
  ...
  CfgScope getTarget(RelevantCall call) {
    exists(string method |
      ...
      exists(Module m, boolean exact | result = lookupSingletonMethod(m, method, exact) |
        ...
        // end
        // ```
        selfInSingletonMethodFlowsToMethodCallReceiver(call, m, method, exact)
      )
    )
    or
...
pragma[nomagic]
private MethodBase lookupSingletonMethodInBaseClasses(Module m, string name) {
  ...
  exists(Module sub | sub.getSuperClass() = m |
    ...
    result = lookupSingletonMethodInBaseClasses(sub, name)
  )
}
...
private MethodBase lookupSingletonMethod(Module m, string name, boolean exact) {
  ...
  exact in [false, true]
  or
  result = lookupSingletonMethodInBaseClasses(m, name) and
  exact = false
}
...
private predicate mayBenefitFromCallContextSingleton(
  ...
  (
    tp = resolveConstantReadAccess(arg.asExpr().getExpr()) and
    exact = true
    or
...
DataFlowCallable viableImplInCallContext(DataFlowCall call, DataFlowCall ctx) {
  ...
  (
    ...
    exists(RelevantCall call0, RelevantCall ctx0, ArgumentNode arg, string name |
      call0 = call.asCall() and
      ctx0 = ctx.asCall() and
      mayBenefitFromCallContext0(ctx0, arg, call0, _, name) and
