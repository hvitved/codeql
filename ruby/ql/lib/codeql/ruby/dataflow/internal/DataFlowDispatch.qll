...
private module TrackModuleInput implements CallGraphConstruction::InputSig {
  class State = Module;
  ...
  Unit stateProj(Module m) { exists(m) and exists(result) }
...
predicate trackModuleAccess = CallGraphConstruction::Make<TrackModuleInput>::track/1;
...
private module TrackInstanceInput implements CallGraphConstruction::InputSig {
  ...
  private predicate localFlowStep(
  ...
  private predicate startState(DataFlow::Node start, Module tp, boolean exact) {
  ...
  newtype State = additional MkState(Module m, boolean exact) { startState(_, m, exact) }

  predicate start(DataFlow::Node start, State state) {
    exists(Module m, boolean exact |
      state = MkState(m, exact) and
      startState(start, m, exact)
    )
  }
...
private module TrackBlockInput implements CallGraphConstruction::InputSig {
  class State = Block;
...
private predicate trackBlock = CallGraphConstruction::Make<TrackBlockInput>::track/1;
...
private module TrackSingletonMethodOnInstanceInput implements CallGraphConstruction::InputSig {
  ...
  newtype State =
    additional MkState(MethodBase method, string name) {
      singletonMethodOnInstance(method, name, _)
    }

  predicate start(DataFlow::Node start, State state) {
    exists(MethodBase method, string name |
      singletonMethodOnInstance(method, name, start.asExpr().getExpr()) and
      state = MkState(method, name)
    )
  }
  ...
  StateProj stateProj(State state) { state = MkState(_, result) }
...
private DataFlow::Node trackSingletonMethodOnInstance(MethodBase method, string name) {
  result =
    CallGraphConstruction::Make<TrackSingletonMethodOnInstanceInput>::track(TrackSingletonMethodOnInstanceInput::MkState(method,
        name))
}
