...
private predicate argMustFlowToReceiver(
  RelevantCall ctx, DataFlow::LocalSourceNode source, DataFlow::Node arg, DataFlowCall call,
  string name
) {
  exists(ParameterNodeImpl p, ParameterPosition ppos, ArgumentPosition apos, DataFlowCallable encl |
    ...
    encl = call.getEnclosingCallable() and
    p.isParameterOf(encl, ppos) and
...
private predicate mayBenefitFromCallContextInitialize(
  ...
  exists(DataFlow::LocalSourceNode source |
    argMustFlowToReceiver(ctx, pragma[only_bind_into](source), arg, TNormalCall(new), "new") and
...
private predicate mayBenefitFromCallContextInstance(
  RelevantCall ctx, DataFlowCall call, DataFlow::Node arg, Module tp, boolean exact, string name
) {
  exists(DataFlow::LocalSourceNode source |
...
private predicate mayBenefitFromCallContextSingleton(
  ...
  exists(DataFlow::LocalSourceNode source |
    argMustFlowToReceiver(ctx, pragma[only_bind_into](source), pragma[only_bind_into](arg),
      TNormalCall(call), pragma[only_bind_into](name)) and
...
predicate mayBenefitFromCallContext(DataFlowCall call) {
  ...
  or
  mayBenefitFromCallContextInstance(_, call, _, _, _, _)
  or
...
DataFlowCallable viableImplInCallContext(DataFlowCall call, DataFlowCall ctx) {
  ...
  (
    ...
    exists(CfgScope res |
      pragma[only_bind_out](res) = result.asCfgScope() and
      exists(Module m, string name |
        exists(RelevantCall call0 |
        ...
        or
        exists(boolean exact |
          mayBenefitFromCallContextInstance(ctx.asCall(), pragma[only_bind_into](call), _,
            pragma[only_bind_into](m), pragma[only_bind_into](exact), pragma[only_bind_into](name)) and
          res = getTargetInstance(call, name) and
          ...
          exists(RelevantCall call0 |
            ...
            mayBenefitFromCallContextSingleton(ctx.asCall(), pragma[only_bind_into](call0), _,
              pragma[only_bind_into](m), pragma[only_bind_into](exact), pragma[only_bind_into](name)) and
            res = getTargetSingleton(call, name) and
    ...
    exists(RelevantCall ctx0, DataFlow::Node arg, string name |
      ctx0 = ctx.asCall() and
      argMustFlowToReceiver(ctx0, _, arg, call, name) and
      not mayBenefitFromCallContextInitialize(ctx0, call.asCall(), arg, _, _) and
      not mayBenefitFromCallContextInstance(ctx0, call, arg, _, _, name) and
      not mayBenefitFromCallContextSingleton(ctx0, call.asCall(), arg, _, _, name) and
      result = viableSourceCallable(call)
    )
    or
    // library calls should always be able to resolve
    argMustFlowToReceiver(ctx.asCall(), _, _, call, _) and
    result = viableLibraryCallable(call)
  )
}
