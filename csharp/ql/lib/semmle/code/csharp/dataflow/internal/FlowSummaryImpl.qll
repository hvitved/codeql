...
module Input implements InputSig<DataFlowImplSpecific::CsharpDataFlow> {
  ...
  string encodeContent(ContentSet c, string arg) {
    ...
    or
    exists(Field f | c = TFieldContent(f) and result = "Field" and arg = f.getQualifiedName())
    or
    exists(Property p |
      c = TPropertyContent(p) and result = "Property" and arg = p.getQualifiedName()
    )
    or
  ...
  string encodeReturn(ReturnKind rk, string arg) {
    result = "Argument" and
    arg = rk.(OutRefReturnKind).getPosition().toString()
  }
...
private module TypesInput implements Impl::Private::TypesInputSig {
  ...
  DataFlowType getParameterType(UnboundCallable c, ParameterPosition pos) {
    exists(Type t | result.asGvnType() = Gvn::getGlobalValueNumber(t) |
      exists(int i |
  ...
  DataFlowType getReturnType(UnboundCallable c, ReturnKind rk) {
    exists(Type t | result.asGvnType() = Gvn::getGlobalValueNumber(t) |
